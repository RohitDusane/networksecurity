...
class DataTransformation:
    ...
    def initiate_data_transformation(self) -> DataTransformationArtifact:
        logging.info(f"Entered initaite_data_transformation methon in class DataTransformation")
        try:
            ...
            # TRAIN DataFrame (Split Feature & Target)
            input_feature_train_df = train_df.drop(columns=[TARGET_COLUMN], axis=1)
            target_feature_train_df = train_df[TARGET_COLUMN]
            target_feature_train_df = target_feature_train_df.replace(-1, 0)

            # TEST DataFrame (Split Feature and Target)
            input_feature_test_df = test_df.drop(columns=[TARGET_COLUMN],axis=1)
            target_feature_test_df = test_df[TARGET_COLUMN]
            target_feature_test_df = target_feature_test_df.replace(-1,0)

            # data transforms using `KNNimputer`
            preprocessor = self.get_data_transfomed_object()
            preporcessor_object = preprocessor.fit(input_feature_train_df)
            transformed_input_feature_train = preporcessor_object.transform(input_feature_train_df)
            transformed_input_feature_test = preporcessor_object.transform(input_feature_test_df)
